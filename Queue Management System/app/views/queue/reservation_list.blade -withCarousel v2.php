<div id="cart">
    <div style="width: 100% !important;" id="myCarousel" class="carousel slide" >
        <div class="col-md-12">
            <div class="box box-primary">
                ...
                <div class="box-body">
                    <div class="carousel-inner">
                        <div class="item active" id="front">
                            <table class="table table-bordered">
                                ...
                                @if($clinics)
                                    @foreach ($clinics as $key => $val)
                                        <?php
                                        $activePhyIds = UsersLocalizationClinics::getActivePhysiciansByClinicId($val, false, false, false, 'user_id');
                                        ?>
                                        @foreach ($activePhyIds as $key2 => $val2)
                                            ...
                                            @foreach($reservations as $key3 => $val3)
                                                ...
                                                <tr @if($val3['patient_status'] == \core\enums\PatientStatus::patient_in)
                                                    ...
                                                    @if($key3 == 0)
                                                        <td style="vertical-align: middle; text-align: center;background: {{$background}};width: 250px;"
                                ...
                                <table class="table table-bordered">
                                    ...
                                    @if($clinics[$clinicKey])
                                        @for($i = $clinicKey; $i < count($clinics); $i++)
                                            <?php
                                            $activePhyIds = UsersLocalizationClinics::getActivePhysiciansByClinicId($clinics[$i], false, false, false, 'user_id');
                                            ?>
...
<script>
    ...
    $("#myCarousel").carousel({interval: 8000, pause: null});
//    $("#back").show();
    @else
    $("#myCarousel").carousel('pause');
//    $("#back").hide();
