...
class PhysicianManager extends BaseManager
{
    ...
    public function updatePhysician($inputs, $id)
    {
        ...
        try {
            ...
            $user->user_specialty_id = $inputs['specialty_id'];
                        ...
                        if (empty($physicianException)) {
                            ...
                            foreach ($physicianException as $key => $val) {
                                if ($val['schedule_times']) {
                                    if ($physicianSchedule) {
                                        $startTime = $physicianSchedule[$daysName[$inputDayName] . '_start_time_1'];
                                        $endTime = $physicianSchedule[$daysName[$inputDayName] . '_end_time_1'];
                                        if (strpos($val['schedule_times'], $startTime . ' ') !== false
                                        && strpos($val['schedule_times'], $endTime) !== false) {
                                            if ($val['effect'] == 1) {
                        ...
                        if ($end > $start) {
                            ...
                            if (empty($physicianException)) {
                                ...
                                foreach ($physicianException as $key => $val) {
                                    if ($val['schedule_times']) {
                                        if ($physicianSchedule) {
                                            $startTime = $physicianSchedule[$daysName[$inputDayName] . '_start_time_2'];
                                            $endTime = $physicianSchedule[$daysName[$inputDayName] . '_end_time_2'];
                                            if (strpos($val['schedule_times'], $startTime . ' ') !== false
                                                && strpos($val['schedule_times'], $endTime) !== false) {
                                                if ($val['effect'] == 1) {
                                                    $availableTimes[$counts]['reserved'] = true;
                            ...
                            if ($end > $start) {
                                ...
                                if (empty($physicianException)) {
                                    ...
                                    foreach ($physicianException as $key => $val) {
                                        if ($val['schedule_times']) {
                                            if ($physicianSchedule) {
                                                $startTime = $physicianSchedule[$daysName[$inputDayName] . '_start_time_3'];
                                                $endTime = $physicianSchedule[$daysName[$inputDayName] . '_end_time_3'];
                                                if (strpos($val['schedule_times'], $startTime . ' ') !== false
                                                && strpos($val['schedule_times'], $endTime) !== false) {
                                                    if ($val['effect'] == 1) {
                                                        $availableTimes[$counts]['reserved'] = true;
